...
export function isVotable(
  ...
  if (anthillGraph[voterId] === undefined) {
    const error = new Error(
      "isVotable called with undefined voter with id: " +
        voterId +
        ", this should not happen, as we should be checking only for clicked and rendered nodes",
    );
    throw error;
  }
...
export function isDagVote(
  ...
  if (anthillGraphBare[recipientId] === undefined) {
    const error = new Error(
      "isDagVote called with undefined recipient with id: " +
        recipientId +
        ", this should not happen, as we should be checking only for clicked and rendered nodes",
    );
    throw error;
  ...
  if (
    voterFull.sentDagVotes === undefined ||
    voterFull.sentDagVotes.length === 0
  ) {
    return false;
  }
...
export function isSwitchable(
  ...
  if (anthillGraph[voterId] === undefined) {
    const error = new Error(
      "isSwitchable called with undefined voter with id: " +
        voterId +
        ", this should not happen, as we should be checking only for clicked and rendered nodes",
    );
    throw error;
  ...
  if (anthillGraphBare[recipientId] === undefined) {
    const error = new Error(
      "isSwitchable called with undefined recipientwith id: " +
        recipientId +
        ", this should not happen, as we should be checking only for clicked and rendered nodes",
    );
    throw error;
  ...
  // console.log("In idDagVote, voter",voter, "recipient", recipient)
  if (
    voterFull.sentTreeVote === recipientId &&
    recipientRep < voterFull.currentRep
  ) {
    return true;
  }
