...
function renderingNodeData(
  ...
  if (node.name === "Name") {
    nodeRendering.name =
      node.id.slice(0, 2) + ".." + node.id.slice(node.id.length - 3);
  } else {
  ...
  if (isAccountInGraph) {
    nodeRendering.isVotable = isVotable(
      account,
      node as NodeDataBare,
      maxRelRootDepth,
      anthillGraph,
      anthillGraphBare,
    );
    if (nodeRendering.isVotable) {
      nodeRendering.isDagVote = isDagVote(
        account,
        node.id,
        maxRelRootDepth,
        anthillGraph,
    ...
    nodeRendering.isSwitchable = isSwitchable(
      account,
      node.id,
      node.currentRep,
      maxRelRootDepth,
...
function renderingNodeDataBare(
  node: NodeDataBare,
  account: string,
  isAccountInGraph: boolean,
  maxRelRootDepth: number,
...
const matchAndSpliceParent: (
  id: string,
  newParentId: string,
  neighbourhood: GraphDataRendering,
) => (parentId: string, i: number) => void =
  (id: string, newParentId: string, neighbourhood: GraphDataRendering) =>
...
export function SelectNeighbourhoodWithParentChain(
  id: string,
  account: string,
  isAccountInGraph: boolean,
  maxRelRootDepth: number,
  ...
  neighbourhood[id] = renderingNodeData(
    node,
    account,
    isAccountInGraph,
    maxRelRootDepth,
  ...
  node.sentDagVotes.forEach((sDagVote) => {
    ...
    neighbourhood[recipient.id] = renderingNodeDataBare(
      recipient,
      account,
      isAccountInGraph,
      maxRelRootDepth,
  ...
  for (let i = 0; i < maxRelRootDepth; i++) {
    if (
      parent.sentTreeVote === "" ||
      parent.sentTreeVote === "0x0000000000000000000000000000000000000001"
    ) {
      break;
    }
    ...
    const newParent: NodeDataRendering = renderingNodeDataBare(
      anthillGraphBare[newParentId],
      account,
      isAccountInGraph,
      maxRelRootDepth,
      anthillGraph,
    ...
    if (neighbourhood[newParentId] !== undefined) {
      ...
      neighbourhood[id].parentIds.map(
        matchAndSpliceParent(id, newParentId, neighbourhood),
      );
    }
  ...
  if (node.recDagVotes.length !== 0) {
    node.recDagVotes.forEach((rDagVote) => {
      ...
      if (voter !== undefined) {
        ...
        neighbourhood[voter.id] = renderingNodeDataBare(
          voter,
          account,
          isAccountInGraph,
          maxRelRootDepth,
  ...
  if (node.recTreeVotes.length !== 0) {
    node.recTreeVotes.forEach((id) => {
      ...
      if (neighbourhood[id] === undefined && voter !== undefined) {
        ...
        neighbourhood[id] = renderingNodeDataBare(
          voter,
          account,
          isAccountInGraph,
          maxRelRootDepth,
...
export function SelectWholeGraph(
  id: string,
  account: string,
  isAccountInGraph: boolean,
  maxRelRootDepth: number,
...
export function SelectSentRecDagVotes(
  id: string,
  account: string,
  isAccountInGraph: boolean,
  maxRelRootDepth: number,
