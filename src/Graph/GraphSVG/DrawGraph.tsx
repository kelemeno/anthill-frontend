...
function GraphDataToArray(graph: GraphDataRendering): NodeDataRendering[] {
  const array: NodeDataRendering[] = [];
  for (const [key] of Object.entries(graph)) {
    const node = graph[key] as NodeDataRendering;
    array.push(node);
  }
  return array;
}

export const DrawGraph = (props: {
  ...
  const dag = reader(GraphDataToArray(dag_data));
  ...
  const nontemplatestring =
    '<svg width="' + width + '" height="' + height + '"></svg>';

  const templateString = `${nontemplatestring}`;

  const svgNode = svg.default(templateString);
  // `<svg width= ${Width} height= ${Height}></svg>`;
  ...
  for (const [, node] of [...dag].entries()) {
    ...
    colorMap[node.data.id] = interp(
      ((Math.sin(stringUniqueHash) / 2 + 1 / 2) * 10) % 1,
    );
  ...
  const nodes = svgSelection
    ...
    .on("click", (n) => {
      props.handleClick(
        n.target.__data__.data.id,
        n.target.__data__.data.name,
        n.target.__data__.data.currentRep,
      );
    })
  ...
  nodes.append("circle").attr("r", (d) => {
    if (d.data.id === props.clickedNode) {
      return nodeRadius + 4;
    } else {
      return 0;
    }
