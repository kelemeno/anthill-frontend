...
export async function CheckSaveNeighbourHoodWithParentChain(
  backendUrl: string,
  id: string,
  ...
  await getBareParentsForDepth(
    backendUrl,
    anthillGraph[id].sentTreeVote,
    2 * maxRelRootDepth,
    anthillGraph,
    anthillGraphBare,
    maxRelRootDepth,
  ...
  await checkSaveBareNodeArray(
    backendUrl,
    anthillGraph[id].recTreeVotes,
    anthillGraph,
    anthillGraphBare,
    maxRelRootDepth,
  );
  await checkSaveBareNodeArray(
    backendUrl,
    anthillGraph[id].sentDagVotes.map((r) => r.id),
    anthillGraph,
    anthillGraphBare,
    maxRelRootDepth,
  );
  await checkSaveBareNodeArray(
    backendUrl,
    anthillGraph[id].recDagVotes.map((r) => r.id),
    anthillGraph,
    anthillGraphBare,
    maxRelRootDepth,
  );
...
async function getBareParentsForDepth(
  backendUrl: string,
  sentTreeVote: string,
  depthDiff: number,
  anthillGraph: GraphData,
  ...
  if (
    sentTreeVote !== "0x0000000000000000000000000000000000000001" &&
    sentTreeVote !== ""
  ) {
    await checkSaveBareNode(
      backendUrl,
      sentTreeVote,
      anthillGraph,
      anthillGraphBare,
    ...
    if (
      depthDiff > 0 &&
      newBareNode.sentTreeVote !== "0x0000000000000000000000000000000000000001"
    ) {
      await getBareParentsForDepth(
        backendUrl,
        newBareNode.sentTreeVote,
        depthDiff - 1,
        anthillGraph,
        anthillGraphBare,
        maxRelRootDepth,
...
async function checkSaveBareNodeArray(
  backendUrl: string,
  ids: string[],
  anthillGraph: GraphData,
  anthillGraphBare: GraphDataBare,
  ...
  for (const i in ids) {
    if (
      ids[i] !== "" &&
      ids[i] !== "0x0000000000000000000000000000000000000001"
    ) {
      await checkSaveBareNode(
        backendUrl,
        ids[i],
...
export async function CheckSaveNode(
  backendUrl: string,
  id: string,
  anthillGraph: GraphData,
  anthillGraphBare: GraphDataBare,
...
async function checkSaveBareNode(
  backendUrl: string,
  id: string,
  anthillGraph: GraphData,
  anthillGraphBare: GraphDataBare,
...
export async function CheckSaveWholeGraph(
  backendUrl: string,
  id: string,
  anthillGraph: GraphData,
  anthillGraphBare: GraphDataBare,
  ...
  await checkSaveChildrenRecursive(
    backendUrl,
    potRoot,
    anthillGraph,
    anthillGraphBare,
...
async function checkSaveChildrenRecursive(
  backendUrl: string,
  id: string,
  anthillGraph: GraphData,
  anthillGraphBare: GraphDataBare,
  ...
  for (const i in anthillGraph[id].recTreeVotes) {
    await checkSaveChildrenRecursive(
      backendUrl,
      anthillGraph[id].recTreeVotes[i],
      anthillGraph,
      anthillGraphBare,
      maxRelRootDepth,
    );
