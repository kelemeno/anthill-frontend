...
const gettingMaxRelRootDepth = async (
  backendUrl: string,
  maxRelRootDepth: number,
  setMaxRelRootDepth: any,
) => {
  if (maxRelRootDepth === 0) {
    getMaxRelRootDepth(backendUrl).then((res) => {
...
const gettingAccount = async (
  ...
  if (isAccountInGraph === true) {
    ...
    await CheckSaveNeighbourHoodWithParentChain(
      backendUrl,
      account,
      anthillGraph,
      anthillGraphBare,
...
const gettingClickedNode = async (
  ...
  if (clickedNode !== "Enter") {
    ...
    if (treeMode) {
      ...
      graphToRender = SelectWholeGraph(
        clickedNode,
        account,
        isAccountInGraph,
        maxRelRootDepth,
    ...
    if (!deepEqual(graphToRender, graphDisplayed)) {
      ...
      if (altNode !== newAltNode) {
        setAltNode(newAltNode);
      }
    }
  }
};
...
export const Graph = (props: {
  ...
  const [graphDisplayed, setGraphDisplayed] = useState({
    id: {
  ...
  } as GraphDataRendering);

  const [altNode, setAltNode] = useState("");

  const clearingGraph = async () => {
    if (props.account !== undefined) {
      await getIsNodeInGraph(props.backendUrl, props.account).then((res) => {
        if (res !== isAccountInGraph) {
          setIsAccountInGraph(res);
        }
      });
    } else {
      if (isAccountInGraph) {
