...
// and the d3 graph renders nodes. (The on-chain *join* write is wallet-gated,
// so it is covered by the Vitest on-chain integration test instead.)
//
...
test.describe("graph read/navigate flow", () => {
  test("boots, fetches a random leaf, and renders the graph", async ({
    ...
    // The graph is drawn into <div class="Graph"> as an <svg> with node circles.
    await expect(page.locator("div.Graph svg").first()).toBeVisible({
      timeout: 15_000,
    });
    await expect
      .poll(() => page.locator("div.Graph svg circle").count(), {
        timeout: 15_000,
      })
      .toBeGreaterThan(0);
